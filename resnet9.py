...
class ResNet9(Base):
    ...
    def forward(self,x,label=None):
        ...
        if label is not None:
            loss = self.criterien(x,label)

        return x,loss

def conv_gn_relu_pool(in_channels, out_channels, pool=False):
    layers = [
        nn.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, padding=1),
        #nn.BatchNorm2d(out_channels),
        nn.GroupNorm(32,out_channels),
...
class ResNet9_group(Base):
    def __init__(self, in_channels, num_classes):
        super().__init__()
        self.prep = conv_gn_relu_pool(in_channels, 64)
        self.layer1_head = conv_gn_relu_pool(64, 128, pool=True)
        self.layer1_residual = nn.Sequential(conv_gn_relu_pool(128, 128), conv_gn_relu_pool(128, 128))
        self.layer2 = conv_gn_relu_pool(128, 256, pool=True)
        self.layer3_head = conv_gn_relu_pool(256, 512, pool=True)
        self.layer3_residual = nn.Sequential(conv_gn_relu_pool(512, 512), conv_gn_relu_pool(512, 512))
        self.MaxPool2d = nn.Sequential(
            nn.MaxPool2d(4))
        self.linear = nn.Linear(512, num_classes)
        self.criterien = nn.CrossEntropyLoss(reduction='none')
...
class Classifier(nn.Module):
    ...
    def forward(self,image,label=None):
        bz = image.shape[0]
        data = self.conv(image)
        data = data.view(bz,-1)
        data = self.mlp(data)

        loss=None
        if label is not None:
            loss = self.criterien(data,label)
