...
if args.enable_wandb:
    wandb.init(project=args.wandb_project, entity="zclfe",name=args.wandb_name,group=args.wandb_name)
...
def train(epoch,global_model,global_model_weights,epoch1=1,epoch2=1,args=None):    
...
for i in range(args.rounds):
    ...
    global_model_weights = train(i,global_model,global_model_weights,1,1,args)
    ...
    if args.enable_wandb:
        wandb.log({'sup_correct':sup_correct.mean().item()})

    logging.info(f'{i},{sup_correct.mean()}')
    if sup_correct.mean().item()>best_acc:
        best_acc=sup_correct.mean().item()
